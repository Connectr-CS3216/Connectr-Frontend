...
angular.module('connectrFrontendApp').directive('mapboxGlMap', function(session, apis, colorPicker) {
  return {
    ...
    link: function($scope) {
      ...
      $scope.$on('map.needsRemoveControl', function() {
        controls.forEach(function(c) {
          c.remove();
        });
      });

      $scope.$on('map.needsDisplayControl', function() {
        controls.forEach(function(c) {
          c.addTo($scope.map);
        });
      });
      ...
      function htmlForFeature(feature) {
        ...
        var html = htmlTemplate(friend.avatar, friend.name, friend.primaryColor, feature.properties.point_count,
          feature.properties['place_name'], timeString);
      ...
      $scope.$on('map.needsAddCheckinsForFriend', function(event, friend) {
        if (!$scope.displayedFriendIDs.includes(friend.id)) {
          ...
          var colors = friend.id === session.currentUser.id ? colorPicker.getColorMatrix() :
            colorPicker.getColorMatrix(friend.id);
